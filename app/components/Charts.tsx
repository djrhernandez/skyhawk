...
import {
	BarChart, XAxis, YAxis, CartesianGrid, Bar, ResponsiveContainer, Tooltip, Text
} from 'recharts'
...
export const Charts = ({ data, pageState }: any, props: any) => {
	...
	const buildingData = Object.keys(buildingList)
		.map((bucket) => ({ bucket, count: buildingList[bucket] }))
		.filter((item) => item.bucket !== 'RH')
		.sort((a, b) => b.count - a.count)

	return (
		<>
			{pageState === 'failure' && renderError()}
...
const renderError = () => (
<div style={{textAlign: 'center', fontSize: '1rem'}}>Error!</div>
)
